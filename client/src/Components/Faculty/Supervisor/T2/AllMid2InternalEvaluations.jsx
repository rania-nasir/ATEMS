...
export default function AllMid2InternalEvaluations({ setShowDetails }) {
    const [thesisData, setThesisData] = useState([]);

    useEffect(() => {
        async function fetchThesisData() {
            try {
                const response = await fetch('http://localhost:5000/faculty/internalthesis2AllMidEvals', {
                    method: 'GET',
                    headers: {
            ...
            <div className="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
                <div className="w-full my-4">
                    <h2 className="text-center text-2xl tracking-tight text-gray-950 font-bold">
                        All Thesis 2 Mid Evaluations For Internals
                    </h2>
                </div>
                            ...
                            {thesisData.map((student) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <NavLink
                                            to={student?.rollno ? `/mid2EvalDetails/${student.rollno}` : '#'}
