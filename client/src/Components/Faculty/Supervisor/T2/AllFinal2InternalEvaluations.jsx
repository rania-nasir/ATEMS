...
export default function AllFinal2InternalEvaluations({ setShowDetails }) {
    const [thesisData, setThesisData] = useState([]);

    useEffect(() => {
        async function fetchThesisData() {
            try {
                const response = await fetch('http://localhost:5000/faculty/internalthesis2AllFinalEvals', {
                    method: 'GET',
                    headers: {
                            ...
                            {thesisData?.map((student) => (
                                ...
                                >
                                    ...
                                    <td className="px-6 py-4">
                                        <NavLink
                                            to={student?.rollno ? `/final2EvalDetails/${student.rollno}` : '#'}
