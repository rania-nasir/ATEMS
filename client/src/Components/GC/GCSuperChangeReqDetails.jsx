...
export default function GCSuperChangeReqDetails() {
    ...
    useEffect(() => {
        async function fetchthesisData() {
            try {
                const response = await fetch(`http://localhost:5000/gc/gcViewPendingSupervisorDetails/${rollno}`, {
                    method: 'GET',
                    headers: {
    ...
    const approveData = async (e) => {
        ...
        const res = await fetch(`http://localhost:5000/gc/ApprovePendingSupervisorChange/${rollno}`, {
            method: "POST",
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `${Cookie.get('jwtoken')}`
        ...
        if (res.status === 200) {
            if (data.message === "Supervisor Change Request Approved") {
                showMessage('success', data.message);
                console.log(data.message);
                setTimeout(() => {
                    navigate('/gcViewPendingSupervisorRequests');
                }, 3000);
            } else {
    ...
    const declineData = async (e) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/gc/RejectPendingSupervisorChange/${rollno}`, {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `${Cookie.get('jwtoken')}`
            ...
            if (res.ok) {
                ...
                if (data.message === "Supervisor Change Request Rejected") {
                    showMessage('success', data.message);
                    console.log(data.message);
                    setTimeout(() => {
                        navigate('/gcViewPendingSupervisorRequests');
                ...
                <div className="mt-2 bg-gray-500 shadow overflow-hidden sm:rounded-lg w-[90%]">
                    <div className="px-4 py-5 sm:px-6">
                        <p className="max-w-2xl text-md text-white">
                            Supervisor Change Request Details
                        </p>
                    </div>
                    {ReqData && (
