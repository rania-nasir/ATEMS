...
export default function GCSuperChangeRequest() {
    const [reqData, setReqData] = useState([]);

    useEffect(() => {
        async function fetchReqData() {
            try {
                const response = await fetch('http://localhost:5000/gc/gcViewPendingSupervisorRequests', {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    console.log(data);
                    setReqData(data.pendingSupervisorChangeRequests); // Update state with thesisData array
                } else {
                        ...
                        {reqData.length > 0 ? (
                            reqData.map(rowData => (
                                <tr className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600" key={rowData.thesisid}>
                                    ...
                                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {rowData.currsupervisorname}
                                    </td>
                                    <td className="px-6 py-4">
                                        <NavLink
                                            to={`/gcViewPendingSupervisorDetails/${rowData.rollno}`}
