...
export default function GCTitleChangeRequestDetails() {
    ...
    useEffect(() => {
        async function fetchthesisData() {
            try {
                const response = await fetch(`http://localhost:5000/gc/gcViewPendingTitleDetails/${rollno}`, {
                    method: 'GET',
                    headers: {
    ...
    const approveData = async (e) => {
        ...
        const res = await fetch(`http://localhost:5000/gc/ApprovePendingTitleRequests/${rollno}`, {
            method: "POST",
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `${Cookie.get('jwtoken')}`
        ...
        if (res.status === 200) {
            if (data.message === "Title Change Request Approved") {
                showMessage('success', data.message);
                console.log(data.message);
                setTimeout(() => {
                    navigate('/gcViewPendingTitleRequests');
                }, 3000);
            } else {
    ...
    const declineData = async (e) => {
        ...
        try {
            const res = await fetch(`http://localhost:5000/gc/RejectPendingTitleRequests/${rollno}`, {
                method: "POST",
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `${Cookie.get('jwtoken')}`
            ...
            if (res.ok) {
                ...
                if (data.message === "Title Change Request Rejected") {
                    showMessage('success', data.message);
                    console.log(data.message);
                    setTimeout(() => {
                        navigate('/gcViewPendingTitleRequests');
