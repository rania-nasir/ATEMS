...
export default function StudentviewAnnouncement() {
    ...
    useEffect(() => {
        async function fetchAnnouncementData() {
            try {
                const response = await fetch('http://localhost:5000/std/viewAnnouncement', {
                    headers: {
                    ...
                    <tbody>
                        {announcementData.length > 0 ? (
                            announcementData.map(rowData => (
                                <tr className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600" key={rowData.announcementTitle}>
