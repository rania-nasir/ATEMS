...
const getAllApprovedThesis = async (req, res) => {
    try {
        ...
        const uniqueThesisTitles = await twomidevaluations.findAll({
            attributes: [
                [sequelize.fn('DISTINCT', sequelize.col('thesistitle')), 'thesistitle']
            ],
            where: {
                gcapproval: 'Ready',
            },
        ...
        if (!uniqueThesisTitles || uniqueThesisTitles.length === 0) {
            return res.json({ message: 'No Ready Thesis found' });
        }

        res.json(uniqueThesisTitles);
...
const assignExternalToThesis = async (req, res) => {
    try {
        ...
        const thesis2MidEvals = await twomidevaluations.findAll({
            where: {
        ...
        if (!thesis2MidEvals || thesis2MidEvals.length === 0) {
            return res.json({ message: 'Thesis not found or not approved' });
        }

        // Check if external IDs are already assigned for all relevant thesis titles
        const allAssigned = thesis2MidEvals.every(Thesis2MidEval => !!Thesis2MidEval.externalid);
        if (allAssigned) {
            return res.json({ message: 'External IDs are already assigned for the given thesis titles' });
        }

        let nextFacultyId;

        // Check if an external with the same email already exists
        const existingExternal = await faculties.findOne({
            where: {
                email: email,
                role: {
                    [Op.contains]: ["External"]
                },
            }
        });

        if (existingExternal) {
            // Use the existing facultyid
            nextFacultyId = existingExternal.facultyid;
        } else {
            ...
            // Create a new external in the faculties table
        ...
        // Update all relevant twomidevaluations records with the same thesistitle
        const promises = thesis2MidEvals.map(async (Thesis2MidEval) => {
        ...
        res.json({ message: "External has been added successfully" });
